...
const App = () => {
  ...
  const useAutoScroll = async () => {
    ...
    if (!scroll) {
      addLog("Γ¥î No Auto Scrolls remaining!");
      return;
    }
    ...
    addLog("ΓÜí Auto Scroll engaged (+1 Hour)");
  };
  ...
  const processKill = () => {
      ...
      if (Math.random() < dropChance) {
        ...
        if (possibleLoots.length > 0) {
          ...
          if (lootItem) {
            ...
            addLog(`≡ƒÄü LOOT: Found ${lootItem.name}!`);
          }
        }
      }
    }
    ...
    if (newKills >= 10) {
       ...
       setDepth(nextDepth);
       addLog(`Γ¼å∩╕Å FLOOR UP! Ascending to Floor ${nextDepth}...`);
