...
const getSellValue = (itemId, itemData) => {
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        if (action === 'BUY_ITEM') {
            ...
            if (!isCounterItem && (currentSlots + qty) > maxSlots) {
                throw new https_1.HttpsError('failed-precondition', `Bag full! You have ${currentSlots}/${maxSlots} slots used. Sell items or upgrade your storage.`);
            }
            const updates = {
        ...
        if (action === 'SELL_ITEM') {
            ...
            const trueSellValue = getSellValue(targetItem.id || itemId, targetItem);
            ...
            if (inventory[itemId]) {
                const targetBaseId = extractBaseId(inventory[itemId].id || itemId);
            ...
            if (removed < sellQty) {
                ...
                for (const [key, invItem] of entries) {
                    ...
                    if (!invItem)
                        continue;
                    const invBaseId = extractBaseId(invItem.id || key);
                    if (invBaseId === baseId) {
                        delete inventory[key];
                        removed++;
                    }
        ...
        if (action === 'UPGRADE_INVENTORY_SLOTS') {
            ...
            if (currentMax >= 120) {
                throw new https_1.HttpsError('failed-precondition', 'Maximum storage capacity of 120 slots has already been reached.');
            }
            const updates = {};
            const inventory = userData.inventory || {};
            ...
            if (method === 'GX') {
                ...
                if (currentMax >= 100)
                    cost = 30000;
                else if (currentMax >= 70)
                    cost = 15000;
                if ((userData.tokens || 0) < cost) {
                    throw new https_1.HttpsError('failed-precondition', 'Insufficient GX Gold. Keep grinding!');
                }
            ...
            // --- 2. THE WEB3 FAST-TRACK (On-Chain Tokens) ---
                ...
                if (!txHash) {
                    throw new https_1.HttpsError('invalid-argument', 'Missing blockchain transaction hash.');
                }
                const txRef = db.collection('usedTransactions').doc(txHash);
                const txSnap = await transaction.get(txRef);
                if (txSnap.exists) {
                    throw new https_1.HttpsError('already-exists', 'This transaction has already been claimed.');
                }
                ...
                if (!isDevEnv) {
                    ...
                    if (!txReceipt || txReceipt.status !== 1) {
                        throw new https_1.HttpsError('failed-precondition', 'Transaction failed or is not yet mined on Base.');
                    }
                    const txData = await provider.getTransaction(txHash);
                    const playerAddress = userData.walletAddress?.toLowerCase();
                    if (txData.from.toLowerCase() !== playerAddress) {
                        throw new https_1.HttpsError('permission-denied', 'Transaction was not initiated by your connected wallet.');
                    }
                    ...
                    if (txData.to?.toLowerCase() !== expectedContract) {
                        throw new https_1.HttpsError('failed-precondition', `Invalid target contract address. Expected token contract: ${expectedContract}`);
                    }
                    ...
                    try {
                        ...
                        if (toAddress !== faucetAddress) {
                            throw new https_1.HttpsError('failed-precondition', `Incorrect payment recipient. Expected faucet address: ${faucetAddress}`);
                        }
                        ...
                        if (rawAmount < requiredAmount) {
                            ...
                            throw new https_1.HttpsError('failed-precondition', `Insufficient payment. Sent ${formattedSent} ${method}, but requires at least ${formattedReq} ${method}.`);
                        }
                    }
                    catch (err) {
                        throw new https_1.HttpsError('failed-precondition', err.message || 'Failed to decode ERC20 transfer payload.');
                    }
                }
                ...
                if (method === 'HUNT' && currentMax >= 100) {
                    ...
                    if (sparkKeys.length < 2) {
                        throw new https_1.HttpsError('failed-precondition', 'Requires at least 2 Hunt Sparks in your bag.');
                    }
                ...
                throw new https_1.HttpsError('invalid-argument', 'Invalid upgrade currency method.');
            }
        ...
        if (action === 'USE_POTION') {
            ...
            if (currentHp >= resolvedMaxHp) {
                throw new https_1.HttpsError('failed-precondition', 'Already at full HP!');
            }
            const potionSpecs = {
                'hp_potion': { mult: 0.1 },
            ...
            if (!spec) {
                throw new https_1.HttpsError('invalid-argument', 'Invalid potion selection.');
            }
            const inventory = userData.inventory || {};
            const updates = {
                updatedAt: admin.firestore.FieldValue.serverTimestamp()
            ...
            if (!hasPotion) {
                throw new https_1.HttpsError('failed-precondition', `No ${selection.replace(/_/g, ' ')}'s found in bag.`);
            }
            if (usedItemId) {
