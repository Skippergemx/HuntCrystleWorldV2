  ...
  async (request) => {
    ...
    try {
      ...
      if (balance < (rewardValue + ethers.parseEther("0.00001"))) { 
         ...
         throw new HttpsError('resource-exhausted', "The town's treasury is currently depleted! Citizens are working to restock the faucet.");
      }
...
export const secureGameAction = onCall(
  { enforceAppCheck: true }, // ENFORCEMENT: Rejects requests without valid App Check token
  async (request) => {
    if (!request.auth || !request.auth.uid) {
  ...
  return await db.runTransaction(async (transaction) => {
    ...
    if (action === 'PROCESS_KILL_REWARDS') {
      ...
      if (loots && loots.length > 0) {
        ...
        loots.forEach((loot: any) => {
          if (loot.id?.includes('_pool_')) return; // handled by counters
    ...
    if (action === 'SELL_ITEM') {
      ...
      const inventory = userData.inventory || {};
      if (!inventory[itemId]) throw new HttpsError('not-found', 'Item not in inventory.');
