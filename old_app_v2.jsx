...
const CRYSTLE_RECIPES = [
  { id: 'ruby_pendant', name: 'Ruby Heart Pendant', cost: 800, stats: { str: 20, dex: 15, agi: 15 }, type: 'Relic', img: '≡ƒÆì' },
  { id: 'emerald_blade', name: 'Emerald Wind-Slayer', cost: 2500, stats: { str: 60, dex: 40, agi: 20 }, type: 'Weapon', img: '≡ƒùí∩╕Å' },
  { id: 'sapphire_plate', name: 'Sapphire Aegis', cost: 4000, stats: { str: 15, dex: 15, agi: 60 }, type: 'Armor', img: '≡ƒ¢í∩╕Å' },
];
...
const App = () => {
  ...
  const handleAttack = (isBoss = false) => {
    ...
    if (p.hiredMate && buffTimeLeft <= 0 && Math.random() < 0.5) {
      ...
      addLog(`Γ£¿ ${mate.name} cast a buff on you!`);
  ...
  const enemyTurn = (target, isBoss = false) => {
    ...
    if (Math.random() * 100 < hitChance) {
      ...
      if (isCrit) { addLog(`ΓÜá∩╕Å CRIT!`); setCritAlert(true); setTimeout(() => setCritAlert(false), 800); setStunTimeLeft(STUN_DURATION_CRIT / 1000); }
                        ...
                        <div className="space-y-1 text-left">
                          ...
                          <div className="bg-slate-100 px-2 py-1 border border-black/10 inline-block">
                             <p className="text-[9px] text-slate-500 font-black uppercase italic leading-none">
                               ...
                               {item.type !== 'Consumable' && ` ΓÇó REQ LVL ${item.reqLvl}`}
                               {item.id === 'hp_potion' && ` ΓÇó STASHED: ${player.potions || 0}`}
                               {item.id === 'auto_scroll' && ` ΓÇó STASHED: ${player.autoScrolls || 0}`}
                             </p>
                          </div>
                        </div>
                      </div>
          ...
          <div className="space-y-1">
            {logs.map((log, i) => (
              <div key={i} className={`text-xs font-black uppercase leading-tight italic ${i === 0 ? 'text-black' : 'text-black/40'}`}>
                {i === 0 ? <span className="mr-2">Γû╢</span> : <span className="mr-2 opacity-50">ΓÇó</span>}
                {log}
              </div>
            ))}
          </div>
