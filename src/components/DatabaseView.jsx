...
export const DatabaseView = React.memo(() => {
  ...
  useEffect(() => {
    const isHidden = localStorage.getItem('hide_database_tutorial') === 'true';
    if (!isHidden) {
  ...
  const tutorialSteps = [
    {
      title: "Archive Infonet",
      npc: 2,
      visualType: 'database',
      text: "Accessing the main database. This terminal contains every known scrap of data regarding the Meta-verse ecosystems.",
      hint: "Tip: Use the search bar to locate specific entities quickly."
    },
    {
      title: "Drop Target Analysis",
      npc: 6,
      visualType: 'loot',
      text: "The Sector Nodes tab actively tracks where materials drop and calculates real-time probability percentages.",
      hint: "Strategy: Cross-reference maps with the material you need."
    },
    {
      title: "Beast Profiling",
      npc: 1,
      visualType: 'beast',
      text: "Studying the Beast Lore tab provides combat stats for every monster you might encounter, helping you prepare your loadout.",
      hint: "Warning: Unlisted boss variants may still exist..."
    }
  ];
  ...
  const nextStep = () => {
    if (tutorialStep < tutorialSteps.length - 1) {
      ...
      if (dontShowAgain) {
        localStorage.setItem('hide_database_tutorial', 'true');
      }
            ...
            <div className="relative bg-slate-900 border-[3px] md:border-[4px] border-black rounded-3xl z-10 flex flex-col items-center overflow-hidden">
              ...
              <div className="py-3 md:py-4 relative flex justify-center items-center gap-3 w-full z-10">
                ...
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-xl border-[3px] border-black overflow-hidden relative shadow-[4px_4px_0_rgba(0,0,0,1)] transform -rotate-2 bg-slate-800 shrink-0 flex items-center justify-center">
                   ...
                   <div className="absolute inset-x-0 bottom-0 bg-blue-600 text-[6px] font-black text-black text-center py-0.5 uppercase italic">SYSTEM</div>
                </div>
                ...
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-xl border-[3px] border-black relative shadow-[4px_4px_0_rgba(255,255,255,0.1)] bg-slate-950 flex items-center justify-center shrink-0 group">
                   {tutorialSteps[tutorialStep].visualType === 'database' && (
                     <Database className="text-blue-400 drop-shadow-[0_0_10px_rgba(59,130,246,0.5)] z-10 animate-bounce" size={40} />
                   )}
                   {tutorialSteps[tutorialStep].visualType === 'loot' && (
                     <Target className="text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)] z-10 animate-pulse" size={40} />
                   )}
                   {tutorialSteps[tutorialStep].visualType === 'beast' && (
                     <Skull className="text-purple-400 drop-shadow-[0_0_10px_rgba(168,85,247,0.5)] z-10 animate-pulse" size={40} />
                   )}
                </div>
              ...
              <div className="px-4 pb-3 w-full relative z-10 flex flex-col min-h-0">
                <div className="bg-white text-black p-3 md:p-3.5 rounded-xl border-[3px] border-black relative mb-3 shadow-[3px_3px_0_rgba(0,0,0,1)] shrink-0">
                  <div className="absolute -top-3 -left-1 bg-blue-400 text-[8px] font-black px-2 py-0.5 border-2 border-black uppercase italic shadow-sm">
                ...
                <div className="flex gap-2 shrink-0 pb-1">
                  ...
                  <button
                    onClick={nextStep}
                    className="flex-[2] bg-blue-600 text-black py-2.5 rounded-xl font-black uppercase tracking-widest hover:bg-blue-500 transition-all border-[3px] border-black shadow-[3px_3px_0_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none italic text-[10px] md:text-xs flex items-center justify-center gap-1.5"
                  >
                    {tutorialStep === tutorialSteps.length - 1 ? 'ACCESS ARCHIVES' : 'TRANSMIT MORE'}
                    <Sparkles size={12} />
                  </button>
                </div>
              </div>
